...
class Df {
    ...
    yearSetting() {
        G.create({
            trigger: ".year-sticky",
            start: ae.isSPView ? "top 110vw" : "top 25%",
            end: ae.isSPView ? "top 110vw" : "top 25%",
            endTrigger: ".p-action-history .list-item:last-child",
            pin: !0,
            pinType: "fixed",
            scroller: ae.scroller()
        });
        const e = 110 / 390 * window.innerWidth / window.innerHeight * 100;
        console.log(e),
            this.list = w.selectorAll(".p-action-history__list [data-year]"),
            this.currentYear = this.list[0].dataset.year,
            this.yearArr = [],
            this.list.forEach((i, r) => {
                const s = i.dataset.year;
                this.yearArr.includes(s) || this.yearArr.push(s)
            }
            );
        for (let i = 0, r = this.yearArr.length; i < r; i++) {
            const s = this.yearArr[i];
            let o = document.createElement("p");
            o.innerHTML = s,
                o.style.transform = `translate3d(${-100 * i}%, 0, 0)`,
                this.year_sticky_inner.appendChild(o)
        }
        const t = new IntersectionObserver(i => {
            i.forEach(r => {
                const s = r.target;
                if (r.isIntersecting) {
                    this.currentYear = s.dataset.year;
                    const o = this.yearArr.indexOf(this.currentYear);
                    this.year_sticky_inner.style.transform = `translate3d(${o * 100}%, 0, 0)`
                }
            }
            )
        }
            , {
                rootMargin: ae.isSPView ? `-${e}% 0% -${100 - e}% 0%` : "-29% 0% -71% 0%",
                threshold: [0, .5, 1]
            });
        this.list.forEach((i, r) => {
            t.observe(i)
...
class If {
    advance(e) {
        ...
        if (this.lerp)
            this.value = (r = this.value,
                s = this.to,
                (1 - (o = 1 - Math.exp(-60 * this.lerp * e))) * r + o * s),
                Math.round(this.value) === this.to && (this.value = this.to,
                    i = !0);
        else {
            this.currentTime += e;
            const a = an(0, this.currentTime / this.duration, 1);
            i = a >= 1;
    ...
    fromTo(e, t, { lerp: i = .1, duration: r = 1, easing: s = a => a, onUpdate: o }) {
    ...
    constructor(e, { wheelMultiplier: t = 1, touchMultiplier: i = 2, normalizeWheel: r = !1 }) {
        this.onTouchStart = s => {
            ...
            this.onWheel = s => {
                ...
                this.normalizeWheel && (o = an(-100, o, 100),
                    a = an(-100, a, 100)),
...
class Ff {
    constructor({ wrapper: e = window, content: t = document.documentElement, wheelEventsTarget: i = e, smoothWheel: r = !0, smoothTouch: s = !1, syncTouch: o = !1, syncTouchLerp: a = .1, __iosNoInertiaSyncTouchLerp: l = .4, touchInertiaMultiplier: c = 35, duration: u, easing: f = v => Math.min(1, 1.001 - Math.pow(2, -10 * v)), lerp: h = u && .1, infinite: d = !1, orientation: m = "vertical", gestureOrientation: p = "vertical", touchMultiplier: g = 1, wheelMultiplier: _ = 1, normalizeWheel: T = !1, autoResize: S = !0 } = {}) {
        this.onVirtualScroll = ({ type: v, inertia: x, deltaX: C, deltaY: b, event: M }) => {
            if (M.ctrlKey)
                return;
            const E = v === "touch"
                , A = v === "wheel";
            if (this.options.gestureOrientation === "vertical" && b === 0 || this.options.gestureOrientation === "horizontal" && C === 0 || E && this.options.gestureOrientation === "vertical" && this.scroll === 0 && !this.options.infinite && b <= 0 || M.composedPath().find(I => (I == null || I.hasAttribute == null ? void 0 : I.hasAttribute("data-lenis-prevent")) || E && (I == null || I.hasAttribute == null ? void 0 : I.hasAttribute("data-lenis-prevent-touch")) || A && (I == null || I.hasAttribute == null ? void 0 : I.hasAttribute("data-lenis-prevent-wheel"))))
                return;
            if (this.isStopped || this.isLocked)
                return void M.preventDefault();
            if (this.isSmooth = (this.options.smoothTouch || this.options.syncTouch) && E || this.options.smoothWheel && A,
                !this.isSmooth)
                return this.isScrolling = !1,
                    void this.animate.stop();
            M.preventDefault();
            let k = b;
            this.options.gestureOrientation === "both" ? k = Math.abs(b) > Math.abs(C) ? b : C : this.options.gestureOrientation === "horizontal" && (k = C);
            const O = E && this.options.syncTouch
                , D = E && x && Math.abs(k) > 1;
            D && (k = this.velocity * this.options.touchInertiaMultiplier),
                this.scrollTo(this.targetScroll + k, fo({
                    programmatic: !1
                }, O && {
                    lerp: D ? this.syncTouchLerp : this.options.__iosNoInertiaSyncTouchLerp
                }))
        }
            ,
            this.onScroll = () => {
    ...
    scrollTo(e, { offset: t = 0, immediate: i = !1, lock: r = !1, duration: s = this.options.duration, easing: o = this.options.easing, lerp: a = !s && this.options.lerp, onComplete: l = null, force: c = !1, programmatic: u = !0 } = {}) {
        if (!this.isStopped || c) {
            ...
            if (typeof e == "number") {
                if (e += t,
                    e = Math.round(e),
                    this.options.infinite ? u && (this.targetScroll = this.animatedScroll = this.scroll) : e = an(0, e, this.limit),
                    i)
...
class Nf {
    ...
    scrollTrigger() {
        this.container = w.selector(".l-container"),
            this.contentMain = w.selector(".l-container main"),
            this.kv = w.selector(".p-kv"),
            this.isLenisUse = !0,
            ae.isSPView && (this.isLenisUse = !1),
            this.isLenisUse ? (ae.isSPView ? this.setLenis(this.container) : this.setLenis(window),
                F.ee.on("kill-scroll", e => {
                    e ? (this.lenis && this.lenis.stop(),
                        w.addClass(this.html, "-no-scroll")) : (this.lenis && this.lenis.start(),
                            w.removeClass(this.html, "-no-scroll"))
                }
                ),
                F.ee.on("set-lenis-wrapper", e => {
                    this.setLenis(e)
                }
                )) : (this.isTop && this.container.addEventListener("scroll", () => {
                    this.container.scrollTop >= this.contentMain.clientHeight - window.innerHeight - 1 && this.resetInview()
                }
                ),
                    F.ee.on("kill-scroll", e => {
                        e ? w.addClass(this.html, "-no-scroll") : w.removeClass(this.html, "-no-scroll")
                    }
                    )),
            F.ee.on("scrollto", e => {
    ...
    setLenis(e) {
        ...
        const r = s => {
            this.lenis.raf(s),
                G.update(),
                this.rafInt = requestAnimationFrame(r),
                this.isTop && this.lenis.scroll >= this.contentMain.clientHeight - window.innerHeight - 1 && this.resetInview()
        }
            ;
        this.rafInt = requestAnimationFrame(r)
    }
    resetInview() {
        this.isLenisUse ? this.lenis.scrollTo(0, {
            duration: 0,
            lerp: 0
            ...
            this.kv.dataset.type = "loop",
            w.selectorAll("[data-text-motion='split']").forEach((i, r) => {
                w.removeClass(i, "-show")
            }
            ),
            w.selectorAll("[data-inview]").forEach((i, r) => {
                w.removeClass(i, "-inview")
            }
            ),
            F.ee.emit("reset-inview")
    }
    resize() {
